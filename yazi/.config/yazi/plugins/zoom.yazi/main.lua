...
local function peek(_, job)
	...
	local auto = job.args[1] == "auto"
	local stored_level = get(Url(url))
	local level = job.new_level or stored_level or tonumber(job.args[1]) or 0
	local sync = function()
	...
	if auto and not stored_level and not job.new_level and info.w > 0 and info.h > 0 then
		local max_scale = math.min(math.floor(max_w / info.w), math.floor(max_h / info.h))
		if max_scale > 1 then
			level = 10 * (max_scale - 1)
		end
	end
