...
Use `auto` to automatically enlarge small images (e.g. pixel art) to fill the preview area with crisp integer scaling:
...
This calculates the largest integer scale factor (2x, 3x, 4x…) that fits within the preview area, making pixel art clearly visible without blurring. You can still use `+`/`-` to adjust the zoom from there.
